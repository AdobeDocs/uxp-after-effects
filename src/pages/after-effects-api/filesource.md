...
| file | *string* | R | 27.0 | The file path for the file that defines this asset. To change the value: if this FileSource is a `proxySource` of an AVItem, call `setProxy()` or `setProxyWithSequence()`; if it's a `mainSource` of a FootageItem, call `replace()` or `replaceWithSequence()`. |
...
| missingFootagePath | *string* | R | 27.0 | The path and filename of footage that is missing from this asset. |
